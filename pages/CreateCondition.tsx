...
import { Home, FilePlus, ArrowLeft, Save, CalendarClock, FileText, ChevronDown, X, Check } from 'lucide-react';
...
const CreateCondition: React.FC = () => {
  ...
  const handleNavigateWithState = (path: string) => {
    ...
    // If we are in view mode with multiple, preserve original structure 
        ...
        {isViewMode ? (
            ...
            <>
                <Input
                    ...
                    required={false}
                    placeholder="1234#12"
                    className="mb-0" // Remove bottom margin since we handle it in container
                />
                {characterName && (
  ...
  return (
    <div className="mx-auto mt-2 px-2 w-full landscape:w-9/12 relative">
       <ConfirmModal 
         isOpen={showConfirm}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-panel border border-gray-300 dark:border-gray-600 overflow-hidden">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-300 dark:border-gray-600 flex justify-between items-center gap-2">
          <h2 className="text-lg font-display font-bold text-gray-800 dark:text-gray-100 truncate">
            {isViewMode ? 'Condition Properties' : 'Create Condition'}
          </h2>
            ...
            {isViewMode ? (
              <div className="flex gap-2">
                <div className="w-20 shrink-0">
                   <Input label="COIN" value={viewCoin} readOnly className="font-mono bg-entity-condition/10 text-entity-condition" />
                </div>
