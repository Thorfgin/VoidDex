  ...
  useEffect(() => {
    if (location.state) {
        if (location.state.initialData) {
             const { condition: savedCondition, expiryDate: savedExpiry, selectedPlins: savedPlins } = location.state.initialData;
    ...
    <div className="mx-auto mt-2 px-2 w-full landscape:w-9/12">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-panel border border-gray-300 dark:border-gray-600 overflow-hidden">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-300 dark:border-gray-600 flex justify-between items-center gap-2">
          <h2 className="text-lg font-display font-bold text-gray-800 dark:text-gray-100 truncate">Extend Condition</h2>
        ...
        <div className="p-4">
           {!condition && (
             <form onSubmit={handleSearch} className="flex flex-col gap-2 max-w-sm mx-auto">
                <Input label="Enter COIN" value={coinSearch} onChange={(e) => setCoinSearch(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="4-digit ID" error={searchError} className="mb-0" inputMode="numeric" />
                <div className="flex justify-end">
                   <Button type="submit" isLoading={isSearching} disabled={!coinSearch}><Search size={16} className="mr-2" /> Find</Button>
                </div>
             </form>
           )}
           {condition && (
